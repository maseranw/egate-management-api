...
def create_support_ticket_type(support_ticket_type: SupportTicketTypeCreate, db: Session = Depends(get_db)):
...
def get_support_ticket_type(id: int, db: Session = Depends(get_db)):
...
def get_all_support_ticket_types(db: Session = Depends(get_db)):
    return SupportTicketTypeService(db).get_all()


@router.put("/support-ticket-type/{id}", response_model=SupportTicketTypeResponse)
def update_support_ticket_type(id: int, support_ticket_type: SupportTicketTypeUpdate, db: Session = Depends(get_db)):
    db_support_ticket_type = SupportTicketTypeService(db).get(id)
    if db_support_ticket_type is None:
        raise HTTPException(status_code=404, detail="Support ticket type not found")
...
def delete_support_ticket_type(id: int, db: Session = Depends(get_db)):
    db_support_ticket_type = SupportTicketTypeService(db).get(id)
    if db_support_ticket_type is None:
        raise HTTPException(status_code=404, detail="Support ticket type not found")
