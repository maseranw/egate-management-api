...
def create_support_ticket(support_ticket: SupportTicketCreate, db: Session = Depends(get_db),auth: AuthJWT = Depends()):
    auth.jwt_required()
...
def get_support_ticket(ticket_id: int, db: Session = Depends(get_db),auth: AuthJWT = Depends()):
    auth.jwt_required()
    ticket_service = SupportTicketService(db)
...
def get_all_support_tickets(db: Session = Depends(get_db),auth: AuthJWT = Depends()):
    auth.jwt_required()
    ticket_service = SupportTicketService(db)
...
def update_support_ticket(ticket_id: int, support_ticket: SupportTicketUpdate, db: Session = Depends(get_db),auth: AuthJWT = Depends()):
    auth.jwt_required()
    ticket_service = SupportTicketService(db)
...
def delete_support_ticket(ticket_id: int, db: Session = Depends(get_db),auth: AuthJWT = Depends()):
    auth.jwt_required()
    ticket_service = SupportTicketService(db)
...
def get_support_tickets_by_tenant_id(tenant_id: int, db: Session = Depends(get_db),auth: AuthJWT = Depends()):
    ticket_service = SupportTicketService(db)
